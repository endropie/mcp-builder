...
export async function validateMCPProject() {
  try {
    ...
    if (
      !package_json.dependencies?.["mcp-builder"] &&
      !package_json.devDependencies?.["mcp-builder"]
    ) {
      throw new Error(
        "This directory is not an MCP project (mcp-builder not found in dependencies or devDependencies)"
      );
    }
  } catch (error) {
